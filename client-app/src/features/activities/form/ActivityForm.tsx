...
export default observer(function ActivityForm() {
    ...
    function handleFormSubmit(activity: ActivityFormValues) {
        if (!activity.id) {
            ...
            updateActivity(newActivity as Activity).then(() => history.push(`/activities/${newActivity.id}`));
        }
    }
            ...
            <Formik validationSchema={validationSchema} enableReinitialize initialValues={activity} onSubmit={values => handleFormSubmit(values)}>
                {({ handleSubmit, isValid, isSubmitting, dirty }) => (
                    <Form className='ui form' onSubmit={handleSubmit} autoComplete='off'>
                        ...
                        <Button as={Link} to='/activities' floated='right' positive type='button' loading={isSubmitting} content='Cancel' />
                    </Form>
                )}
            </Formik>
